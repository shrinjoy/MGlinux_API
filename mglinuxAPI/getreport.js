module.exports = {
  ...
  gerreportbydate: function (db, sql) {
    return new Promise((resolve, reject) => {
      db.query(`Select  [GAMEID],[TICKETTOTALRS],[TARMINALCLS],[TICKETTOTALRS],[TICKETNUMBER],CONVERT(VARCHAR, [GAMETIME], 105) + ' ' + CONVERT(VARCHAR, [GAMETIME], 108) AS GAMETIME,[WINRS],[GAMERESULT] from ticket99 where gamedate=CONVERT(DATE,'${sql['date']}') and tarminalid='${sql['username']}'  order by INTNUMBER DESC`).then((data) => {
        arraydata = []
