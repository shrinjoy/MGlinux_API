            ...
            db.query(`SELECT DATEDIFF(SECOND,  GETDATE(),CONVERT(DATETIME, NEXTDRAW, 109)) AS timer FROM dbo.TARMINALTIMEZONE;`)
                .then((data) => {
                    ...
                    if ((data.recordset[0].timer - 10) > 12) {
                        ...
                        userdata = db
                            .query(
                                `SELECT * from [nrdeluxe].[dbo].[CLIENTLOGIN] where CLIENTUSERNAME ='${req["username"]}' and CLIENTPASSWORD='${req["password"]}'`
                            )
                            .then((data) => {
                                ...
                                if (data.recordset[0].CLIENTBALANCE > req["totalbet"]) {
                                    ...
                                    db.query(
                                        `UPDATE [nrdeluxe].[dbo].[CLIENTLOGIN]  set CLIENTBALANCE  = CLIENTBALANCE -${req["totalbet"]} where CLIENTUSERNAME ='${req["username"]}' and CLIENTPASSWORD='${req["password"]}'`
                                    )
                                        .then((data) => {
                                            ...
                                            db.query("SELECT REPLACE(CAST(ABS(CAST(CHECKSUM(NEWID()) AS BIGINT) * CAST(RAND() * 1000000 AS BIGINT)) AS VARCHAR), '.', '') as barcode")
                                                .then((data) => {
                                                    ...
                                                    var querystring = `INSERT INTO [nrdeluxe].[dbo].[TICKET99] (TICKETNUMBER,TICKETDETAILS,TICKETRS,TICKETTOTALRS,GAMEDATE,GAMETIME,TARMINALID,GAMEID) 
