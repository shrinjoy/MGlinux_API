...
var usermacid = getJSONCookie("macid");
...
if(usermacid ==null)
{
  ...
  var macobj = {
    ...
    macid:generateRandomNumber(111111,999999).toString()
  }
  
setJSONCookie("macid",macobj,3650)
}

usermacid = getJSONCookie("macid");

console.log(usermacid);
...
function checklogin() {
  ...
  alert(usermacid.macid);
  if (captchaInput === captcha) {
    axios({
      method: "post",
      url: "http://193.203.163.194:3000/canlogin_macid",
      data: {
        userid: document.getElementById("uname").value.toString(),
        username: name,
        password: pass,
        macid:usermacid.macid
      },
    })
      .then(function (res) {
        console.log(res.data.data.message);
        if ((res.data.status = "200" && res.data.data.message!="macid_wrong")) {
        ...
        {
        showpopup("contact office", "green");
