module.exports={
    ...
    gerreportbydate:function(db,sql)
    {
        return new Promise((resolve,reject)=>{
            db.query(`Select  [GAMEID],[TICKETTOTALRS],[TICKETTOTALRS],[TICKETNUMBER],FORMAT([GAMETIME], 'dd-MM-yyyy HH:mm:ss')as GAMETIME,[WINRS],[GAMERESULT] from ticket99 where gamedate=CONVERT(DATE,'${sql['date']}') and tarminalid='${sql['username']}'  order by INTNUMBER DESC`).then((data)=>{
              arraydata = []
    ...
    {
        return new Promise((resolve,reject)=>{
            db.query(`Select  [GAMEID],[TICKETTOTALRS],[TICKETTOTALRS],[TICKETNUMBER],FORMAT([GAMETIME], 'dd-MM-yyyy HH:mm:ss')as GAMETIME,[WINRS],[GAMERESULT]from ticket99 where GAMEID='${sql['gameid']}' and gamedate=CONVERT(DATE,'${sql['date']}') and tarminalid='${sql['username']}'  order by INTNUMBER DESC`).then((data)=>{
              arraydata = []
              data.recordsets.forEach(element => {
                arraydata.push(element);
              });
