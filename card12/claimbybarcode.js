module.exports = {
    claim: function (sql, req) {
        return new Promise((resolve, reject) => {
            sql.query(`SELECT * FROM "TICKET12CARD" where "TICKETNUMBER"='${req["barcode"]}' and "TARMINALCLS"='WIN' and TARMINALID='${req["userid"]}'`)
                .then((data) => {
                    if (data.recordset.length < 1) {
                        ...
                        sql.query(`UPDATE "TICKET12CARD" SET "TARMINALCLS"='CLAIMED' ,claimdate =GETDATE() where "TICKETNUMBER"='${req["barcode"]}' and TARMINALID='${req["userid"]}'`)
                            .then(() => {
                                sql.query(`UPDATE "CLIENTLOGIN" SET CLIENTBALANCE = CLIENTBALANCE+${parseInt(winrs) || 0} where "CLIENTID"='${req["userid"]}'`)
                                    .then(() => {
                                        sql.query(`SELECT * FROM "TICKET12CARD" where "TICKETNUMBER"='${req["barcode"]}' and TARMINALID='${req["userid"]}'`)
                                        .then((data)=>{
